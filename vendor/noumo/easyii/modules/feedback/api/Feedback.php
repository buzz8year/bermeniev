...
class Feedback extends \yii\easyii\components\API
{
    ...
    public function api_form($options = [])
    {
      ...
      echo $form->field($model, 'name')->label(false)->textInput(array('placeholder' => 'Enter your name', 'class' => 'form-control form-cstm'));
      echo $form->field($model, 'email')->input('email')->label(false)->textInput(array('placeholder' => 'Enter your mail', 'class' => 'form-control form-cstm'));

      //if($settings['enablePhone']) echo $form->field($model, 'phone');
      //if($settings['enableTitle']) echo $form->field($model, 'title');

      echo $form->field($model, 'text')->textArea(array('placeholder' => 'Enter your message', 'class' => 'form-control form-cstm'))->label(false);

      if($settings['enableCaptcha']) echo $form->field($model, 'reCaptcha')->widget(ReCaptcha::className());

      echo Html::submitButton(Yii::t('easyii', 'SEND'), ['class' => 'btn btn-cstm']);
      ActiveForm::end();
