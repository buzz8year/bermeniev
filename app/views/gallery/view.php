...
<div class="cat-wrap text-right">
  ...
  <a class="center-block <?=($lbm->title == $album->model->title) ? 'active' : '' ?>" href="<?= Url::to(['gallery/all']) ?>">
      ALL&nbsp;
  </a><br/>

  <?php foreach($years as $lbm) : ?>
      <a class="center-block <?=($lbm->title == $album->model->title) ? 'active' : '' ?>" href="<?= Url::to(['gallery/year', 'slug' => $lbm->slug]) ?>">
          <?= $lbm->title ?>
      </a><br/>
  <?php endforeach; ?>
...
<div class="main-wrap port-wrap">
  ...
  <?php if(count($photos)) : ?>
      ...
      <?php if(count($photos) > 1): ?>
        <div class="photo-show jsonly"><a href="javascript:" onclick="$('.gallery-wrap').find('.photo-wrap:first-of-type').find('a').click();"><i class="fa fa-camera" style=""></i><small>slide show</small></a></div>
      <?php endif; ?>
  ...
  <div class="col-xs-12 prev-next">
    <?php if ($years[$k + 1]) : ?>
      <a class="pull-left" href="/gallery/year/<?=$years[$k+1]->slug?>">« <small>PREV</small></a>
    <?php endif; ?>
    <?php if ($years[$k - 1]) : ?>
      <a class="pull-right" href="/gallery/year/<?=$years[$k-1]->slug?>"><small>NEXT</small> »</a>
    <?php endif; ?>
  </div>
